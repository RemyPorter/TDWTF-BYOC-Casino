...
int main(void)
{
	...
	printf("Enter player's cards to prime the engine...\n");
...
again:
		char* input = readline(": ");
		uint8_t myCards[POKER_CARDS_PER_HAND] = {0};
		int index = 0;
		static const char* values = "23456789XJQKA";
		...
		for(int c=0; c < POKER_CARDS_PER_HAND; c++) {
			...
			p = strchr(values, toupper(input[index++]));
			if(!p || !*p) {
				printf("Mitä vittu?!?");
				goto again;
			} else {
			...
			p = strchr(suits, toupper(input[index++]));
			if(!p || !*p) {
				printf("Mitä vittu?!?");
				goto again;
			} else {
				myCards[c] += 13 * (p - suits);
			}
		}

		if(uncertainty > 24) {
			printf("Can't predict yet...\n");

			for(int h=POKER_CARDS_PER_HAND-1; h >= 0; h--)
				dealer.state += dealer.state + myCards[h];

			uncertainty -= 5;
			continue;
		}

		uint64_t stateBackup = dealer.state;
		uint8_t  shift = 64 - uncertainty;
		uint32_t iters = 1 << uncertainty;

		while(iters--) {
			dealer.state = stateBackup ^ (((uint64_t) iters) << shift);
			dealer.Deal();
			if(!memcmp(dealer.hands[0], myCards, POKER_CARDS_PER_HAND)) {
				uncertainty = 0;
				break;
			}
		}
